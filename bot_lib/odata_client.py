...
class ODataClient:
    ...
    def _encode_params(params: dict[str, Any]) -> str:
        """Кодировать query-параметры с ``%20`` вместо ``+`` для пробелов.

        httpx и ``urlencode`` по умолчанию кодируют пробелы как ``+``,
        но 1С OData воспринимает ``+`` как literal плюс, а не пробел.
        Используем ``quote_via=quote`` чтобы пробелы кодировались как ``%20``.
        """
    ...
    def _build_url_with_params(base_url: str, path: str, params: Optional[dict[str, Any]]) -> str:
        ...
        url = f"{base_url}{path}"
        if params:
            url = f"{url}?{ODataClient._encode_params(params)}"
        return url
    ...
    async def _request_raw(
        ...
        *,
        ...
        Query-параметры предварительно кодируются через
        :meth:`_encode_params` (пробелы → ``%20``), чтобы 1С OData
        корректно их обрабатывал.
