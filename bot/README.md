...
Пользователь
    │ вопрос на русском
    ▼
[ИИ — Шаг 1]  STEP1_SYSTEM
    │ JSON: entity, filter, select, orderby, top
    ▼
[OData 1С]  HTTP GET /odata/standard.odata/...
    │ JSON-массив записей
    ▼
...
Файл `bot/master_prompt.md` — системный промпт для шага форматирования ответа (Шаг 2). Редактируйте его для изменения стиля ответов без перезапуска бота — промпт перечитывается при каждом `/refresh`.

Если файл отсутствует, используется встроенный промпт `STEP2_SYSTEM` из `bot.py`.

**Текущий промпт предписывает:**
