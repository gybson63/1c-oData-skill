...
|--------------|------------------------|-------------------|--------------|
| Список работающих с должностью и подразделением | Штатные сотрудники | `email-list-employees` | `InformationRegister_КадроваяИсторияСотрудников/SliceLast()` |
...
| Расшифровка ссылок | Настройка полей отчёта | `email-reference-labels` | `InformationRegister_КадроваяИсторияСотрудников/SliceLast()` + lookup по _Key |
...
| Диалог в цепочке («сколько?» → «какой?») | Отдельный чат в треде | `email-zup-thread-dialog` | история + ответы бота в треде |
| График численности по подразделениям | Штатная расстановка | `email-analytics-chart` | PNG-вложение, analytics (`flaky`, `RUN_FLAKY_TESTS=1`) |
| Сводка по подразделениям таблицей | Численность (без графика) | `email-analytics-table` | VisualizationAdvisor → только таблица |

Цепочка писем ведёт себя как **отдельный чат**: история диалога хранится в `HistoryManager` по `conversation_id` треда, ответы бота дописываются в `EmailThreadStore`, follow-up передаётся агенту коротким текстом («Какой?»), а не всей перепиской целиком.
