...
- Запросы к 1С **обычным письмом**: IMAP/SMTP, ответ HTML + вложения (таблицы, графики)
- Тот же мультиагентный пайплайн (аналитик → OData → форматирование), диалог в **цепочке** писем
- Префикс **`[analyze]`** — аналитик метаданных; whitelist отправителей в конфиге

Подробнее: **[`docs/email.md`](docs/email.md)**.

### Данные 1С на естественном языке

- Telegram-бот: вопрос на русском → данные из базы 1С без ручных запросов
...
| Аудитория | Куда идти |
|-----------|-----------|
| **Email-интерфейс** | **[`docs/email.md`](docs/email.md)** |
| Пользователь бота (Telegram / email) | [`docs/user-guide.md`](docs/user-guide.md) |
